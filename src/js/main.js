...
(function () {
  ...
  window.addEventListener("scroll", function () {
    ...
    var navbar = document.getElementById("featureNav");

    if (featuresSection.getBoundingClientRect().bottom <= 0) {
  ...
  // Function to add active class to the navigation item based on the currently visible section
function setActiveNavItem() {
  const sections = document.querySelectorAll('section[id]');
  
  sections.forEach(section => {
    const navItem = document.querySelector(`#featureNav a[href="#${section.id}"]`);
